...
/**
 * This class is responsible for tracking all currently open activities. By
 * doing so this class can detect when the application is in the foreground and
 * when it is running in the background.
 */
public class AppForegroundStateManager {
	private static final String TAG = AppForegroundStateManager.class
			.getSimpleName();
	private static final int MESSAGE_NOTIFY_LISTENERS = 1;
	public static final long APP_CLOSED_VALIDATION_TIME_IN_MS = 3 * DateUtils.SECOND_IN_MILLIS; // 3
																									// Seconds
	...
	public void onActivityNotVisible(Activity activity) {
		/*
		 * The foreground activity may have been replaced with a new foreground
		 * activity in our app. So only clear the foregroundActivity if the new
		 * activity matches the foreground activity.
		 */
		if (mForegroundActivity != null) {
	...
	/**
	 * Call to determine the current state, update the tracking global, and
	...
	private void determineAppForegroundState() {
		...
		final boolean isInForeground = mForegroundActivity != null
				&& mForegroundActivity.get() != null;
		mAppForegroundState = isInForeground ? AppForegroundState.IN_FOREGROUND
				: AppForegroundState.NOT_IN_FOREGROUND;
		/*
		 * If the new state is different then the old state the notify
		 * subscribers of the state change
		 */
	 ...
	 * @param listener
	 */
	public void addListener(OnAppForegroundStateChangeListener listener) {
		mListeners.add(listener);
	}

	/**
	 * Remove a listener from being notified of app foreground state change
	 * events.
	 * 
	 * @param listener
	 */
	...
	private void notifyListeners(AppForegroundState newState) {
		android.util.Log.i(TAG,
				"Notifying subscribers that app just entered state: "
						+ newState);
	...
	/**
	 * This method will notify subscribes that the foreground state has changed
	 * when and if appropriate. <br>
	 * <br>
	 * We do not want to just notify listeners right away when the app enters of
	 * leaves the foreground. When changing orientations or opening and closing
	 * the app quickly we briefly pass through a NOT_IN_FOREGROUND state that
	 * must be ignored. To accomplish this a delayed message will be Sent when
	 * we detect a change. We will not notify that a foreground change happened
	 * until the delay time has been reached. If a second foreground change is
	 * detected during the delay period then the notification will be canceled.
	 */
	private void validateThenNotifyListeners() {
		// If the app has any pending notifications then throw out the event as
		// the state change has failed validation
		if (mHandler.hasMessages(MESSAGE_NOTIFY_LISTENERS)) {
			...
			if (mAppForegroundState == AppForegroundState.IN_FOREGROUND) {
				// If the app entered the foreground then notify listeners right
				// away; there is no validation time for this
				mHandler.sendEmptyMessage(MESSAGE_NOTIFY_LISTENERS);
			} else {
				// We need to validate that the app entered the background. A
				// delay is used to allow for time when the application went
				// into the
				// background but we do not want to consider the app being
				// backgrounded such as for in app purchasing flow and full
				// screen ads.
		...
		public void handleMessage(Message inputMessage) {
			...
			case MESSAGE_NOTIFY_LISTENERS:
				...
				android.util.Log.v(TAG,
						"App just changed foreground state to: "
								+ mAppForegroundState);
